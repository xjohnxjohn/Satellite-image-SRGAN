...
parser.add_argument('--scale_factor', default=4, type=int, choices=[2, 4, 8], help='super resolution scale factor') # change 
parser.add_argument("--g_every", type=int, default=1, help="train the generator every g_every batch")
parser.add_argument("--d_every", type=int, default=5, help="train the discriminator every d_every batch")
parser.add_argument("--plot_every", type=int, default=20, help="plot using visdom every plot_every batch")
parser.add_argument("--save_every", type=int, default=1, help="save the model every save_every epoch")
...
train_dataloader = DataLoader(
    TrainImageDataset("../../Datasets/My_dataset/single_channel/%s" % opt.train_dataset_name, hr_shape=hr_shape, scale_factor = opt.scale_factor),                # change
...
val_dataloader = DataLoader(
    ValImageDataset("../../Datasets/My_dataset/single_channel/%s" % opt.val_dataset_name, hr_shape=hr_shape, scale_factor = opt.scale_factor),                # change
...
results = {'loss_G': [], 'loss_D': [], 'loss_GAN': [],'loss_content': [], 'loss_real': [], 'loss_fake': [], 'psnr': [], 'ssim': []}     # change

epoch_start = time.time()
for epoch in range(opt.epoch, opt.n_epochs):
    training_results = {'batch_sizes': 0, 'loss_G': 0, 'loss_D': 0, 'loss_GAN': 0, 'loss_content': 0, 'loss_real': 0, 'loss_fake': 0}    # change
    ...
    for i, imgs in enumerate(train_dataloader):
        ...
        # batch_time = time.time() - start
        # print(
        #     "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [batch time: %.3fs]"
        #     % (epoch, opt.n_epochs, i, len(train_dataloader), loss_D.item(), loss_G.item(), batch_time)
        # )
        ...
        if i % opt.plot_every == 0:
            imgs_lr = nn.functional.interpolate(imgs_lr, scale_factor=opt.scale_factor)
            vis.images(imgs_lr.detach().cpu().numpy()[:4] * 0.5 + 0.5, win='imgs_lr_train')
            vis.images(gen_hr.data.cpu().numpy()[:4] * 0.5 + 0.5, win='img_gen_train')
            vis.images(imgs_hr.data.cpu().numpy()[:4] * 0.5 + 0.5, win='img_hr_train')
    ...
    generator.eval()
    out_path = 'training_results/SR_factor_' + str(opt.scale_factor) + '/' + 'epoch_' + str(epoch) + '/'
    ...
    with torch.no_grad():
        ...
        for image in val_save_bar:
            ...
            save_image(image, out_path + 'index_%d.png' % index, padding=5, normalize=True)
            index += 1
    ...
    if epoch % opt.save_every == 0:
        ...
        data_frame.to_csv(out_path + 'SR_factor_' + str(opt.scale_factor) + '_train_results.csv', index_label='Epoch')
